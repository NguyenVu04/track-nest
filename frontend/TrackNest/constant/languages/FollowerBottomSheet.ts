export const Vietnamese = {
  lastActive: "Hoạt động lần cuối",
  now: "bây giờ",
  reportMissing: "Báo cáo mất tích",
  reportSubmitted: "Báo cáo đã được gửi",
  thankYou: "Cảm ơn bạn đã báo cáo.",
  historyTitleWithName: "{{name}} — Lịch sử vị trí",
  noLocationHistoryAvailable: "Không có lịch sử vị trí",
  filterHistoryTitle: "Lọc lịch sử vị trí",
  filterHistoryButton: "Lọc lịch sử vị trí",
  refreshHistoryButton: "Làm mới lịch sử vị trí",
  closeHistoryButton: "Đóng lịch sử vị trí",
  dayLabel: "Ngày",
  fromLabel: "Từ",
  centerOnStart: "Về điểm đầu",
  pointSingular: "điểm",
  pointPlural: "điểm",
  activeNow: "Đang hoạt động",
  currentLocation: "VỊ TRÍ HIỆN TẠI",
  resolvingAddress: "Đang tải địa chỉ...",
  battery: "PIN",
  notAvailable: "Không có sẵn",
  nameLabel: "TÊN",
  coordinatesLabel: "TỌA ĐỘ",
  lastUpdatedLabel: "CẬP NHẬT LẦN CUỐI",
  sos: "SOS",
  sosConfirmTitle: "Gửi cảnh báo khẩn cấp",
  sosConfirmMessage: "Điều này sẽ yêu cầu hỗ trợ khẩn cấp cho {{name}}. Tiếp tục?",
  sosConfirm: "Gửi SOS",
  sosCancel: "Hủy",
  sosSentSuccess: "Đã gửi cảnh báo khẩn cấp",
  sosSentError: "Không thể gửi cảnh báo",
  stationary: "ĐỨNG YÊN",
  walking: "ĐI BỘ",
  driving: "LÁI XE",
};

export const English = {
  lastActive: "Last Active",
  now: "now",
  reportMissing: "Report missing",
  reportSubmitted: "Report submitted",
  thankYou: "Thank you for your report.",
  historyTitleWithName: "{{name}} — Location History",
  noLocationHistoryAvailable: "No location history available",
  filterHistoryTitle: "Filter Location History",
  filterHistoryButton: "Filter location history",
  refreshHistoryButton: "Refresh location history",
  closeHistoryButton: "Close location history",
  dayLabel: "Day",
  fromLabel: "From",
  centerOnStart: "Center start",
  pointSingular: "point",
  pointPlural: "points",
  activeNow: "Active Now",
  currentLocation: "CURRENT LOCATION",
  resolvingAddress: "Resolving address...",
  battery: "BATTERY",
  notAvailable: "Not available",
  nameLabel: "NAME",
  coordinatesLabel: "COORDINATES",
  lastUpdatedLabel: "LAST UPDATED",
  sos: "SOS",
  sosConfirmTitle: "Send Emergency Alert",
  sosConfirmMessage: "This will request emergency assistance for {{name}}. Continue?",
  sosConfirm: "Send SOS",
  sosCancel: "Cancel",
  sosSentSuccess: "Emergency alert sent",
  sosSentError: "Failed to send alert",
  stationary: "STATIONARY",
  walking: "WALKING",
  driving: "DRIVING",
};
