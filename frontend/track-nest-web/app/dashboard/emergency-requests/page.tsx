...
export default function EmergencyRequestsPage() {
  ...
  const handleAccept = async (request: EmergencyRequestResponse) => {
    try {
      ...
      addNotification({
        ...
        description: t("notifAcceptedDesc", { id: request.id }),
        reportId: request.id,
      });
    } catch (error) {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      addNotification({
        ...
        description: t("notifRejectedDesc", { id: rejecting.id.substring(0, 8).toUpperCase() }),
  ...
  const handleClose = async () => {
    ...
    try {
      ...
      addNotification({
        type: "emergency",
        title: t("notifClosedTitle"),
        description: t("notifClosedDesc", { id: completing.id.substring(0, 8).toUpperCase() }),
  ...
  return (
    <div className="max-w-screen-2xl mx-auto pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">{t("hubTitle")}</h1>
          ...
          <div className="overflow-x-auto flex-1">
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-50/50 text-[10px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-100">
                <tr>
                  <th className="px-8 py-4">{t("tableSender")}</th>
                          ...
                          <DropdownMenuContent align="end" className="w-40 rounded-xl shadow-lg border-gray-100 p-1">
                             ...
                             {request.status === "ACCEPTED" && user.role.includes("Emergency Service") && (
                                <DropdownMenuItem onClick={() => setCompleting(request)} className="rounded-lg cursor-pointer font-medium text-blue-700 focus:bg-blue-50 focus:text-blue-800">
                                   <ClipboardCheck className="w-4 h-4 mr-2" /> {t("statusLabelResolved")}
                                </DropdownMenuItem>
                             )}
                          </DropdownMenuContent>
                        </DropdownMenu>
             ...
             <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <span className="text-xs font-bold text-gray-700">
                    {isBroadcasting ? t("broadcasting") : t("broadcastLoc")}
                  </span>
                </button>
             </div>
          </div>
      ...
      {completing && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-xl max-w-md w-full overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <label className="block text-sm font-bold text-gray-700 mb-1">{t("closeNoteLabel")}{tCommon("requiredSuffix")}</label>
              <textarea
