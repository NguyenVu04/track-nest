...
import { emergencyOpsService, AdminEmergencyRequestResponse } from "@/services/emergencyOpsService";
...
export default function AdminEmergencyRequestsPage() {
  ...
  const mapMarkers = filteredRequests.filter(r => r.status === "PENDING" || r.status === "ACCEPTED").map(r => ({
      position: [r.targetLastLatitude, r.targetLastLongitude] as [number, number],
